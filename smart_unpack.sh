...
# ==============================================================================
# SMART UNPACK - Unpacks tar archives with path transformation
# ==============================================================================
# CONFIGURATION
# ==============================================================================
...
# 2. EXTRACTION WITH PATH TRANSFORMATION
# ------------------------------------------------------------------------------
echo "[1/2] Extracting archives with path correction..."
...
extract_archive() {
    ...
    # --transform='s,^,/,' prepends "/" to all paths in the archive
    # This converts "scratch/pawsey1149/..." to "/scratch/pawsey1149/..."
    # Use pushd/popd instead of subshell to handle set -e better
    pushd "$TARGET_DIR" > /dev/null
    tar -xPf "$archive_file" --transform='s,^,/,'
    popd > /dev/null
...
if [ -n "$EXTRACT_DIR" ] && [ -d "$EXTRACT_DIR" ]; then
    ...
    # Check for any "scratch" directories in current directory (sign of failed transform)
    if [ -d "scratch" ]; then
